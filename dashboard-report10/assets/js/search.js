...
const allDamages = damages.filter(d => !(d.deleted === true)); // 最初に1回取得
  ...
  if (deleteBtn) {
    ...
    deleteBtn.onclick = async () => {
        ...
        if (confirm('この損傷データを削除しますか？\n（この操作はリロードすると元に戻ります）')) {
            ...
            const damageToMarkDeleted = damages.find(x => x.id === d.id);
            if (damageToMarkDeleted) {
                damageToMarkDeleted.deleted = true;
            }
            // await saveOverride(d.id, { deleted: true }); // ★ 削除
...
function updateDisplay() {
  ...
  // ★ 修正: getMergedDamages() -> damages.filter(...)
  const matched = damages.filter(d => !(d.deleted === true)).filter(d =>
  ...
  matched.forEach(d => {
    ...
    div.textContent = `${d.type} / ${d.severity} / ${d.inspectionTime} / ${getStatusLabel(d.status)}`; // ★ getStatusLabel は移植済み
...
if (selectedDamage) {
  ...
  const initial = damages.find(x => x.id === selectedDamage.id);
  if (initial) {
